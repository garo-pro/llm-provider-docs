...
Now let's perform similarity search to find the two most similar images in our knowledge base. We do this by embedding a user-provided `image_path` and retrieving the indices and similarity scores for images in our database. A larger score indicates greater similarity, so we sort the scores in descending order.
...
We use the indices to search `image_directory` and select the corresponding images to pass to the vision model for RAG.
