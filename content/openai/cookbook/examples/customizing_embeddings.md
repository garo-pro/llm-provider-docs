...
Most inputs are here. The key things to change are where to load your dataset from, where to save a cache of embeddings to, and which embedding engine you want to use.
...
Note that it's important to split data into training and test sets *before* generating synthetic negatives or positives. You don't want any text strings in the training data to show up in the test data. If there's contamination, the test metrics will look better than they'll actually be in production.
