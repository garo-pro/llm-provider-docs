...
URL Source: https://transformer-circuits.pub/2024/july-update
...
We now properly turn to the question of [multidimensional features](https://arxiv.org/abs/2405.14860) or [feature manifolds](https://transformer-circuits.pub/2023/may-update/index.html#feature-manifolds).

Mathematical linearity requires a multidimensional feature to obey two properties, as mentioned above. Composition as Addition requires that the feature be able to be added to other features. This means the manifold it forms must be orthogonal or (under superposition) almost orthogonal to other features.
...
I'm not at all confident that any of the definitions or frames above will stand up to the test of time. Definitions can and should change and be fluid in research.[Proofs and Refutations](https://en.wikipedia.org/wiki/Proofs_and_Refutations) on how it's often very productive and important for definitions to evolve – if you haven't read it, it's fantastic.
...
From this perspective, these rare features may be a kind of "dark matter" of interpretability
...
Put another way, these tables summarize two vocab_size × vocab_size matrices. The first matrix (the QK circuit) quantifies the amount that any query token attends to a key token in the context (pre-softmax, using the attention “scores” or logits). The other matrix (OV circuit) measures the effect that completely attending to a given key token has on the models’ output logits
...
Once we form these two large matrices, we score each key token according to the following heuristic function 
